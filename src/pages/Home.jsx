...
import staticChannels from '../data/channels'
import staticShorts from '../data/shorts'
import staticReviews from '../data/reviews'
import staticFaq from '../data/faq'
import staticMainVid from '../data/mainVid'
...
export default function HomePage() {
  ...
  const convexShorts = useQuery(api.content.getShorts);
  const convexProjects = useQuery(api.content.getProjects);
  const convexChannels = useQuery(api.content.getChannels);
  const convexReviews = useQuery(api.content.getReviews);
  const convexFaq = useQuery(api.content.getFaq);
  const convexMainVid = useQuery(api.content.getMainVid);

  const shorts = convexShorts ?? staticShorts;
  const projects = convexProjects ?? staticProjects;
  const channels = convexChannels ?? staticChannels;
  const reviews = convexReviews ?? staticReviews;
  const faq = convexFaq ?? staticFaq;
  const mainVid = convexMainVid ?? staticMainVid;
    ...
    <div className="bg-[#070708] text-[#F3F4F4] selection:bg-[#3e46fb] selection:text-white min-h-screen font-sans overflow-x-hidden md:pl-[68px]">
        ...
        {activeVideo && (
          ...
          >
            <motion.div
              ...
              className={`relative w-full ${activeVideo.embedUrl ? 'max-w-sm' : 'max-w-6xl'} bg-[#0c0c0e] border border-zinc-800 rounded-[30px] overflow-hidden shadow-2xl`}
              onClick={(e) => e.stopPropagation()}
            >
              <div className="absolute inset-0 bg-black/40 pointer-events-none z-10" />
              <div className={`${activeVideo.embedUrl ? 'aspect-[9/16]' : 'aspect-video'} w-full bg-black relative z-20`}>
                ...
                <div className="flex items-center gap-5 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    {activeVideo.meta ? (
                      ...
                      <p className="text-[#656df7] text-[11px] font-mono mt-0.5 truncate">{activeVideo.platform} SHORT</p>
                    )}
                  </div>
                </div>
      ...
      <nav className="fixed left-0 top-0 bottom-0 w-[68px] z-[200] hidden md:flex flex-col items-center justify-center gap-1 bg-[#0c0c0e]/72 backdrop-filter backdrop-blur-xl border-r border-white/5">
        <div className="absolute top-6 left-1/2 -translate-x-1/2 font-mono font-black text-xs text-[#3e46fb]">
          <img src="./mcLogo.png" alt="Software Icons" className="w-10 h-10 object-contain" />
        </div>
          ...
          <div className="relative z-30 w-11/12 lg:w-[80%] mx-auto flex flex-col items-center text-center pt-16 pb-8 lg:pt-20 lg:pb-4">
            ...
            >
              <div className="inline-flex items-center">
                <img src="./mcLogo.png" alt="Software Icons" className="w-20 h-20 object-contain" />
              </div>
            </motion.div>
            ...
            >
              <div className="rounded-[30px] overflow-hidden border border-zinc-800 bg-[#0c0c0e] p-4 framer-shadow-premium relative">
                {heroPlaying ? (
                  <div className="w-full aspect-video rounded-[20px] overflow-hidden bg-black">
                    <iframe
                      src={`https://www.youtube.com/embed/${mainVid.id}?autoplay=1`}
                      allow="autoplay; encrypted-media; fullscreen"
                      allowFullScreen
                      className="w-full h-full"
                    />
                  ...
                  <div className="relative w-full aspect-video rounded-[20px] overflow-hidden bg-[#0c0c0e] cursor-pointer" onClick={() => setHeroPlaying(true)} data-cursor="media">
                    <img src={mainVid.preview} alt={mainVid.title} className="w-full h-full object-cover" />
                    <div className="absolute inset-0 flex items-center justify-center">
                      <div className="size-16 rounded-full bg-[#3e46fb] text-white flex items-center justify-center shadow-lg shadow-[#3e46fb]/30 hover:scale-105 transition-transform">
                        <Play size={26} className="fill-current text-white ml-0.5" />
                      </div>
                    </div>
                  </div>
                )}
              ...
              <div className="relative w-full max-w-5xl h-[420px] md:h-[520px] flex items-center justify-center">
                ...
                <motion.div
                  ...
                  onClick={() => {
                    ...
                    if (item.platform === 'YT#') {
                      const id = item.url.match(/(?:shorts\/)([\w-]{11})/)?.[1];
                      setActiveVideo({ embedUrl: `https://www.youtube-nocookie.com/embed/${id}?autoplay=1`, title: item.title, platform: item.platform });
                    } else {
            ...
            {reviews.map((review, idx) => (
              ...
              >
                ...
                <div className="border-t border-zinc-800 pt-4 flex items-center gap-3">
                  {review.logo && (
                    <div className="size-10 rounded-full overflow-hidden border border-zinc-700 shrink-0 bg-[#0c0c0e]">
                      <img src={review.logo} alt={review.author} className="w-full h-full object-cover" />
                    </div>
                  )}
                  <div className="min-w-0">
                    <div className="font-mono text-xs font-black text-white uppercase tracking-tight truncate">
                      {review.author}
                    </div>
                    <div className="font-mono text-[10px] text-[#656df7] mt-0.5 font-bold truncate">
                      {review.role}
                    </div>
                  </div>
                </div>
      ...
      {/* --- CONNECT CTA --- */}
      <section id="contact" className="py-24 border-b border-zinc-800 bg-[#0c0c0e]">
        <div className="w-11/12 lg:w-[80%] mx-auto text-center">
          <span className="text-[#3e46fb] font-mono text-xs tracking-[0.3em] uppercase block font-bold">// NETWORK CONNECTION</span>
          <h2 className="text-4xl md:text-6xl font-black uppercase tracking-tighter text-white font-mono mt-4">
            Connect <span className="text-[#3e46fb]">w/ Me</span>
          </h2>
          <p className="text-zinc-500 text-sm mt-4 max-w-lg mx-auto font-sans">
            Follow the workflow across platforms. Direct messages open for project inquiries.
          </p>
          <div className="flex items-center justify-center gap-6 mt-10">
            <a
              href="https://www.instagram.com/web.mikeey"
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center gap-3 bg-[#070708] border border-zinc-800 hover:border-[#3e46fb]/50 text-white px-8 py-4 rounded-full font-mono text-sm font-bold uppercase tracking-wider transition-all duration-200 hover:bg-[#3e46fb]/5 hover:shadow-lg hover:shadow-[#3e46fb]/10"
              data-cursor="interactive"
            >
              <Instagram size={20} className="text-[#3e46fb]" />
              Instagram
            </a>
            <a
              href="https://x.com/web_mikey"
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center gap-3 bg-[#070708] border border-zinc-800 hover:border-[#3e46fb]/50 text-white px-8 py-4 rounded-full font-mono text-sm font-bold uppercase tracking-wider transition-all duration-200 hover:bg-[#3e46fb]/5 hover:shadow-lg hover:shadow-[#3e46fb]/10"
              data-cursor="interactive"
            >
              <Twitter size={20} className="text-[#3e46fb]" />
              X (Twitter)
            </a>
          </div>
        </div>
        ...
        <div className="w-11/12 lg:w-[80%] mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-12 gap-12 mb-16">
            ...
            <div className="md:col-span-4 space-y-4">
              <img src="./mcLogo.png" alt="MIKEY" className="w-14 h-14 object-contain" />
              ...
              <div className="flex gap-4">
                <a href="https://www.instagram.com/web.mikeey" target="_blank" rel="noopener noreferrer" className="p-2 border border-zinc-850 hover:border-[#3e46fb] rounded-full transition-colors text-zinc-400 hover:text-white" data-cursor="interactive">
                  <Instagram size={16} />
                </a>
                <a href="https://x.com/web_mikey" target="_blank" rel="noopener noreferrer" className="p-2 border border-zinc-850 hover:border-[#3e46fb] rounded-full transition-colors text-zinc-400 hover:text-white" data-cursor="interactive">
                  <Twitter size={16} />
                </a>
              </div>
            </div>
            ...
            <div className="md:col-span-4 space-y-3">
              <p className="text-white uppercase tracking-widest text-[10px] font-bold text-[#68ff1c]">[ CONNECT ]</p>
              <ul className="space-y-2">
                <li>
                  <a href="https://www.instagram.com/web.mikeey" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-zinc-400 hover:text-white transition-colors">
                    <Instagram size={14} className="text-[#3e46fb]" />
                    @web.mikeey
                  </a>
                </li>
                <li>
                  <a href="https://x.com/web_mikey" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-zinc-400 hover:text-white transition-colors">
                    <Twitter size={14} className="text-[#3e46fb]" />
                    @web_mikey
                  </a>
                </li>
              </ul>
